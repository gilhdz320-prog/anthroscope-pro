  ...
  return (
    <div className="space-y-6">
        ...
        <CardContent className="p-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <p className="text-[#55576b] text-sm mt-1">{atletas.length} athletes | {new Date().toLocaleDateString()}</p>
            </div>
            <div className="flex gap-2">
              <Button variant="outline" size="sm" className="border-slate-600 text-[#f0f0f5] hover:bg-[#1e1f2e]" onClick={handleExportExcel}>
                <FileSpreadsheet className="w-4 h-4 mr-1" /> Excel
              </Button>
              <Button variant="outline" size="sm" className="border-slate-600 text-[#f0f0f5] hover:bg-[#1e1f2e]" onClick={handlePrint}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card className="bg-[#22c55e]/10 border-[#D4FF00]/30">
          <CardContent className="pt-6 text-center">
            <Target className="w-6 h-6 text-[#D4FF00] mx-auto mb-2" />
            <p className="text-3xl font-bold text-[#050608]">{optimos}</p>
            <p className="text-xs text-[#D4FF00] font-medium">{t('grupos.optimo')}</p>
          </CardContent>
        </Card>
        <Card className="bg-[#f59e0b]/10 border-amber-200">
          <CardContent className="pt-6 text-center">
            <TrendingUp className="w-6 h-6 text-amber-600 mx-auto mb-2" />
            <p className="text-3xl font-bold text-[#f59e0b]">{atencion}</p>
            <p className="text-xs text-amber-600 font-medium">{t('grupos.atencion')}</p>
          </CardContent>
        </Card>
        <Card className="bg-[#ef4444]/10 border-red-200">
          <CardContent className="pt-6 text-center">
        ...
        <Card>
          <CardContent className="pt-6 text-center">
            <p className="text-xs text-[#8a8d9e]">{t('grupos.grasaProm')}</p>
            <p className="text-3xl font-bold">{promedioGrasa.toFixed(1)}%</p>
            <p className="text-xs text-[#55576b]">IMO: {promedioIMO.toFixed(2)}</p>
          </CardContent>
        </Card>
      </div>

      <Tabs defaultValue="lista" className="space-y-4">
        <TabsList className="bg-[#11121a] border">
            ...
            <Table>
              ...
              <TableBody>
                {atletasFiltrados.map((a) => (
                  <TableRow key={a.id} className="hover:bg-[#0a0b0f] cursor-pointer" onClick={() => { setSelectedAtleta(a); setDialogOpen(true); }}>
                    <TableCell className="font-mono text-xs">{a.id}</TableCell>
                    <TableCell>
                      <div className="font-medium">{a.nombre}</div>
                      <div className="text-xs text-[#55576b]">{a.edad} {t('grupos.anos')} | {a.estatura}cm | {a.masa}kg</div>
                    </TableCell>
                    <TableCell>{a.posicion}</TableCell>
                    <TableCell>{a.imc.toFixed(1)}</TableCell>
                    <TableCell className={a.porcentajeGrasa > 15 ? 'text-red-600 font-semibold' : a.porcentajeGrasa < 8 ? 'text-[#6366f1] font-semibold' : ''}>
                    ...
                    <TableCell>
                      <Badge className={
                        a.cuadrante === 'Muscle High / Fat Low' ? 'bg-[#22c55e]/100 text-[#f0f0f5]' :
                        a.cuadrante === 'Optimal Zone' ? 'bg-[#6366f1]/100 text-[#f0f0f5]' :
                        a.cuadrante === 'Muscle Low / Fat Low' ? 'bg-[#f59e0b]/100 text-[#f0f0f5]' :
                        a.cuadrante === 'Muscle High / Fat High' ? 'bg-[#f59e0b]/100 text-[#f0f0f5]' :
                        'bg-[#0a0b0f]0 text-[#f0f0f5]'
        ...
        <TabsContent value="cuadrantes">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {selectedAtleta ? (
                    ...
                    {['Muscle High / Fat Low', 'Optimal Zone', 'Muscle Low / Fat Low', 'Muscle High / Fat High', 'Neutral Zone', 'Muscle Low / Fat High'].map((cuad, i) => {
                      ...
                      return (
                        <div key={cuad} className="flex items-center gap-3">
                          ...
                          <div className="flex-1 h-3 bg-[#11121a] rounded-full overflow-hidden">
              ...
              <Card className="col-span-2 p-12 text-center text-[#55576b]">
          ...
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="p-4 border rounded-lg bg-[#0a0b0f]">
                  <h4 className="font-bold text-[#f0f0f5] mb-2">{ref.codigo}</h4>
                  <p className="text-sm text-[#8a8d9e] mb-4">{ref.nombre}</p>
                  <div className="space-y-2 text-sm">
                    ...
                    <div className="flex justify-between"><span>Fuente:</span><span className="text-xs text-[#55576b]">{ref.fuente}</span></div>
                  </div>
                </div>

                <div className="p-4 border rounded-lg">
                  <h4 className="font-bold text-[#f0f0f5] mb-2">Team vs {ref.codigo}</h4>
                  ...
                  <div className="mt-4 p-3 bg-[#f59e0b]/10 rounded-lg text-sm">
                    <p className="font-semibold text-[#f59e0b]">{t('resultados.interpretacion')}:</p>
                ...
                <Table>
                  ...
                  <TableBody>
                    {REFERENCIAS.map(r => {
                      ...
                      return (
                        <TableRow key={r.codigo} className={r.codigo === refSeleccionada ? 'bg-[#6366f1]/10' : ''}>
                          ...
                          <TableCell>
                            <span className={diffGrasa < 0 ? 'text-[#D4FF00]' : 'text-amber-600'}>
                              Grasa {diffGrasa < 0 ? 'mejor' : 'mayor'} ({diffGrasa > 0 ? '+' : ''}{diffGrasa.toFixed(1)}%)
                            </span>
                            <br />
                            <span className={diffIMO > 0 ? 'text-[#D4FF00]' : 'text-amber-600'}>
            ...
            <>
              <DialogHeader>
                <DialogTitle className="flex items-center gap-2">
                  ...
                  <Badge className={
                    selectedAtleta.estado === 'optimo' ? 'bg-[#22c55e]/100' :
                    selectedAtleta.estado === 'atencion' ? 'bg-[#f59e0b]/100' : 'bg-[#ef4444]/100'
                  }>
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div className="p-3 bg-[#0a0b0f] rounded"><span className="text-[#8a8d9e]">{t('sujeto.fechaNacimiento')}:</span> <span className="font-semibold">{selectedAtleta.edad} {t('grupos.anos')}</span></div>
                <div className="p-3 bg-[#0a0b0f] rounded"><span className="text-[#8a8d9e]">{t('grupos.pos')}:</span> <span className="font-semibold">{selectedAtleta.posicion}</span></div>
                <div className="p-3 bg-[#0a0b0f] rounded"><span className="text-[#8a8d9e]">{t('mediciones.estatura')}:</span> <span className="font-semibold">{selectedAtleta.estatura}cm</span></div>
                <div className="p-3 bg-[#0a0b0f] rounded"><span className="text-[#8a8d9e]">{t('mediciones.masaCorporal')}:</span> <span className="font-semibold">{selectedAtleta.masa}kg</span></div>
                <div className="p-3 bg-[#0a0b0f] rounded"><span className="text-[#8a8d9e]">IMC:</span> <span className="font-semibold">{selectedAtleta.imc}</span></div>
                <div className="p-3 bg-[#0a0b0f] rounded"><span className="text-[#8a8d9e]">% Fat:</span> <span className="font-semibold">{selectedAtleta.porcentajeGrasa}%</span></div>
                <div className="p-3 bg-[#0a0b0f] rounded"><span className="text-[#8a8d9e]">{t('resultados.masaMuscular')}:</span> <span className="font-semibold">{selectedAtleta.masaMuscular}kg</span></div>
                <div className="p-3 bg-[#0a0b0f] rounded"><span className="text-[#8a8d9e]">IMO:</span> <span className="font-semibold">{selectedAtleta.imo}</span></div>
                <div className="p-3 bg-[#0a0b0f] rounded"><span className="text-[#8a8d9e]">{t('resultados.somatotipo')}:</span> <span className="font-semibold">{selectedAtleta.somatotipo}</span></div>
                <div className="p-3 bg-[#0a0b0f] rounded"><span className="text-[#8a8d9e]">{t('cuadrantes.titulo')}:</span> <span className="font-semibold">{selectedAtleta.cuadrante}</span></div>
              </div>
              <div className="mt-4">
                <CuadrantesHolway
                  masaMuscular={selectedAtleta.masaMuscular}
  ...
  return (
    <div className="space-y-1">
      <div className="flex justify-between text-sm">
        ...
        <span className={esBueno ? 'text-[#D4FF00]' : 'text-amber-600'}>
          Tu equipo: {tuValor.toFixed(1)}{unidad} | Ref: {refValor.toFixed(1)}{unidad}
          <span className="ml-1">({diff > 0 ? '+' : ''}{diff.toFixed(1)})</span>
        </span>
      </div>
      <div className="h-6 bg-[#11121a] rounded-full overflow-hidden relative flex">
        <div className="h-full bg-emerald-400 flex items-center justify-end pr-1 text-[10px] text-[#f0f0f5] font-bold transition-all" style={{ width: `${(tuValor / maxVal) * 100}%` }}>
          Tu equipo
        </div>
        <div className="absolute top-0 h-full border-l-2 border-dashed border-slate-500" style={{ left: `${(refValor / maxVal) * 100}%` }}>
          <span className="absolute -top-4 -left-2 text-[10px] text-[#8a8d9e]">Ref</span>
        </div>
      </div>
    </div>
  );
